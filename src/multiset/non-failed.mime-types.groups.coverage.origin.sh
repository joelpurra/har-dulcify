...
read -d '' getNonFailedMimeTypes <<-'EOF' || true
{
	...
	"kinds-resource-groups": .successfulOrigin.origin.coverage."kinds-resource".groups
}
EOF
...
"${BASH_SOURCE%/*}/../util/dataset-query.sh" "$@" -- test -e "$aggregatesAnalysisJson" '&&' cat "$aggregatesAnalysisJson" '|' jq --arg path '"$PWD"' "'$getNonFailedMimeTypes'" >"datasets.non-failed.mime-types.groups.coverage.origin.json"

<"datasets.non-failed.mime-types.groups.coverage.origin.json" jq "$mapData" | "${BASH_SOURCE%/*}/../util/to-array.sh" | jq "$sortObjects" >"datasets.non-failed.mime-types.groups.coverage.origin.sorted.json"

<"datasets.non-failed.mime-types.groups.coverage.origin.sorted.json" jq "$renameForTsvColumnOrdering" | "${BASH_SOURCE%/*}/../util/array-of-objects-to-tsv.sh" | "${BASH_SOURCE%/*}/../util/clean-tsv-sorted-header.sh" >"datasets.non-failed.mime-types.groups.coverage.origin.sorted.tsv"
